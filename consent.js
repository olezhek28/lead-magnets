/**
 * Cookie consent + Yandex.Metrika для лендинга менторства.
 * Подключается через: <script src="/consent.js" defer></script>
 *
 * Логика:
 *   - Метрика грузится СРАЗУ при загрузке скрипта — фиксируем визит
 *   - Параллельно показывается баннер: Принять / Отклонить / Настроить
 *   - «Принять» — закрываем баннер, ничего не меняем
 *   - «Отклонить» — закрываем баннер, чистим _ym_* cookies (юзер становится
 *     анонимным для последующей навигации в текущей сессии)
 *   - «Настроить» — модалка с тогглами. «Ок» применяет: если Аналитические
 *     выключены — чистим cookies
 *   - Выбор сохраняется в sessionStorage (только на текущую сессию браузера).
 *     При повторном заходе баннер показывается заново.
 */
(function () {
  'use strict';
