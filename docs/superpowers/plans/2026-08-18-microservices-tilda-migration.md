...
Кроме заголовка и текста секция содержит **три карточки со скриншотами вакансий** (Ozon Tech, Wildberries, Aviasales) — каждая открывает попап с полноразмерным скриншотом. Картинки лежат в `microservices/vacancies/`: короткие карточки `ozon.jpg`, `wildberries.jpg`, `aviasales.jpg` и полные версии `*_full.jpg`. В попапах текста нет, только картинка.

Попапы строим тем же универсальным механизмом, что и остальные модалки страницы (`data-modal-open` / `data-modal-close`), который появится в Task 7. Если Task 7 ещё не выполнен на момент вёрстки, разметку попапов всё равно закладываем, а JS подключится позже — как это уже сделано с `data-goal`.

Под карточками — сноска «Скриншоты вакансий сделаны на hh.ru», текст дословно из контент-файла.
...
for slug in ['hero', 'segments', 'what-you-get', 'vacancies']:
    for line in section(slug).split('\n'):
        ...
        # структурно-редакторские строки: rec-ссылки, подписи с двоеточием,
        # обратные кавычки и стрелки — это разметка контент-файла, а не текст страницы
        if re.search(r'rec\d+', line) or line.rstrip().endswith(':') or '`' in line or '\u2192' in line:
            continue
        n = norm(line)
...
# ВАЖНО: это ПРОВЕРКА-ПОДСКАЗКА, а не жёсткий гейт.
# Контент-файл содержит и дословный текст страницы, и редакторские пометки
# («Заголовок секции (две строки):», «переносим как есть», описания попапов).
# Машинно они не различимы, поэтому список ниже всегда будет непустым.
# Гейт устроен так: исполнитель ОБЯЗАН в отчёте расклассифицировать каждую
# оставшуюся строку — «редакторская пометка» либо «реальный пропуск контента», —
# а ревьюер обязан эту классификацию перепроверить выборочно.
# Молча списать строку в пометки нельзя: именно так теряется контент.
print("OK (список выше требует ручной классификации в отчёте)")
PY
```
...
print("OK (оставшиеся строки классифицировать в отчёте: пометка или реальный пропуск)")
print("OK")
PY
```
...
print("OK (каждую оставшуюся строку классифицировать в отчёте; ревьюер перепроверяет)")
print("OK")
PY
```
