...
- `student_stories/` — 12 историй, аватарки участников;
...
По образцу `system_design/index.html:1795-1837`. Заголовок `<h1>` — «Микросервисы, как в BigTech 2.0» (в контент-файле он разбит на строки, разбивку сохранить через `<br>`). Подзаголовок, CTA-кнопка «Записаться на курс» с `href="#tariffs"`, картинки из `hero/`.
...
JS — универсальный, он же обслужит 12 историй в Task 8:
...
- [ ] **Step 4: Stories — 12 историй участников**

Заголовок «Реальные истории участников». Двенадцать карточек: аватарка из `student_stories/`, имя, телеграм-ник, короткая выжимка. Клик открывает модалку с полной историей.

Модалки строим тем же шаблоном, что и `#homework-modal` в Task 7, по одной на историю: `id="story-1"` … `id="story-12"`. JS уже написан и ничего не требует, кнопке достаточно `data-modal-open="story-N"`.

Двенадцать почти одинаковых блоков — это осознанно: сборщиков в проекте нет, генерировать разметку скриптом на клиенте нельзя (контент должен быть в HTML). Пишем разметку целиком.
...
Имена для сверки: Алексей, Олег (дотнет), Олег (бывший фронт), Валера, Кирилл, Антон, Валентина, Руслан, Артур, Дима, Владимир — плюс двенадцатая из контент-файла.
...
Дополнительно проверить, что все 12 модалок на месте и связаны:
...
assert len(stories) == 12, "историй должно быть 12"
...
- модалка ДЗ и 12 модалок историй: открываются, закрываются по Esc / фону / крестику, фокус возвращается на кнопку;
