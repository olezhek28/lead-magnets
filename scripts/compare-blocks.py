...
BLOCKS = [
    ('.blocks .block', 'Знакомая ситуация · сцена 01',
     'Переписана только финалка: «лишние месяцы на старом грейде» → «те, кто пошёл и завалил, через полгода уже сеньоры».'),
    ('#cena-voprosa', 'Между тобой и грейдом',
     'Новый блок между болью и механикой. Полоса скринов вакансий под тоглом vacancy-shots — здесь показана, на сайте пока скрыта.'),
    ('.mech', 'Почему не получается пройти сисдиз',
     'Внешний враг переписан, добавлена врезка «зачем этот цирк», цитата уехала в следующий блок. Видеопродажник — из main, его включил Олег.'),
    ('#chto-ocenivayut', 'Что на самом деле происходит на секции',
     'Новый блок: маршрут секции одной сеткой — что делает интервьюер, где на этом шаге ломаются, в каком уроке чиним. Мост от болей к программе.'),
    ('.prog-head', 'Программа · подводка',
     'Добавлен абзац отстройки: наш рынок, нужные кейсы, недушный язык. Без сравнения с конкурентами.'),
    ...
    ('#avtor .case', 'Автор · продовый кейс',
     'Добавлено «и сам их провожу как интервьюер»: обе стороны стола.'),
    ('#tarify', 'Тарифы',
